...
function addon:DeleteCorruptedScans(lastScan)
	if not self.db.global.settings.scans.autoCleanup.corrupted then
		return
	end
...
function addon:InitializeDatabase()
	...
	backup = backup or self.db.global.backup
	version = version or self.db.global.backup and self.db.global.backup.database

	if backup then
		self.db.global.backup = backup
