...
function addon:SelectExportScans()
    ...
    selectGuild:SetCallback("OnValueChanged", function(self)
        ...
        local i = 1
        addon.tpairs(addon.db.global.guilds[self:GetValue()].scans, function(scans, scanID)
            ...
            i = i + 1
        end, 0.001, _, _, function(a, b) return b < a end)
    end)
    ...
    start:SetCallback("OnClick", function()
        ...
        if not addon.export.guildID then return end
        ...
        local i = 1
        local msg = ""
        addon.tpairs(addon.export.pending, function(scans, scanID)
            if not addon.export.processing then return end
            ...
            if addon.db.global.guilds[addon.export.guildID].scans[scanID] then
                ...
                for tab, tabInfo in pairs(addon.db.global.guilds[addon.export.guildID].scans[scanID].tabs) do
                    ...
                    for _, transaction in pairs(tabInfo.transactions) do
                        ...
                        tinsert(line, date(addon.db.global.settings.preferences.dateFormat, addon:GetTransactionDate(scanID, transactionInfo.year, transactionInfo.month, transactionInfo.day, transactionInfo.hour)))
                ...
                for _, transaction in pairs(addon.db.global.guilds[addon.export.guildID].scans[scanID].moneyTransactions) do
                    if not addon.export.processing then return end
            ...
            if i == addon.tcount(addon.export.pending) then
                ...
                local header = table.concat({"guildName", "snapshotDate", "tabName", "transactionType", "name", "itemName", "itemMoneyCount", "moveTabName1", "moveTabName2", "transactionDate", "transactionLine"}, addon.db.global.settings.preferences.exportDelimiter) 
